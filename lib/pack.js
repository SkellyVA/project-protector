...
async function main() {
  const password = process.env.PROJECT_PASSWORD || (process.stdin.isTTY ? await promptPassword() : '');
  if (!password) {
    console.error('❌ Error: Password is required. Provide PROJECT_PASSWORD environment variable (e.g. for PM2/Docker) or run in an interactive terminal.');
    process.exit(1);
  }

  let decryptedBuffer;
  try {
    decryptedBuffer = decryptPayload(CONTAINER, password);
  } catch (err) {
...
export function packProject(projectDir, outputPath, password, userEntry = null) {
  ...
  const encryptionResult = encryptData(compressedBuffer, password);
  ...
  const container = {
    ...
    entry,
    payload: encryptionResult.payload,
  };
  ...
  return {
    ...
    compressedSize: Buffer.from(encryptionResult.payload, 'base64').length,
  };
}
